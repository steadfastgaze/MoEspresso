...
def build_ds4_iqk_expert_allocations(
    member_by_layer_proj: dict[int, dict[str, str]],
    *,
    layout: str = IQK_LAYOUT_IK_WIRE,
) -> list[dict]:
    """Render IQ_K routed-expert allocation rows for the package plan.

    Every routed cell carries its own member, so a mixed allocation needs no
    special handling anywhere downstream: the row names the member and the
...
def build_ds4_iqk_plan(
    subject: dict,
    iqk_expert_allocation: list[dict],
    *,
