...
The streaming runtime loads one expert's packed/scales bytes from the package
...
Invariants: loading preserves encoded bytes.
"""
...
def _package(tmp_path, n_exp=6, out=8, cols=4):
    ...
    comps = write_bundle_package(pkg, n_layers=1, n_exp=n_exp, out=out,
                                 cols=cols, seed=3)[0]
    return pkg, comps[("gate_proj", "packed")], comps[("gate_proj", "scales")]
...
def test_loaded_scales_match_assembled_component(tmp_path):
    pkg, _, scales = _package(tmp_path)
    idx = build_expert_index(pkg)
    for e in (1, 4):
        got = load_expert(pkg, idx, layer=0, expert=e, projection="gate_proj",
                          component="scales")
        assert np.array_equal(np.array(got), scales[e])
...
def test_loader_reads_every_projection_through_row_geometry(tmp_path):
    ...
    pkg = tmp_path / "pkg"
    pkg.mkdir()
    comps = write_bundle_package(pkg, n_layers=1, n_exp=4, out=6, cols=4,
                                 seed=7)[0]
    idx = build_expert_index(pkg)
    e = 2
...
def test_loader_uses_index_shard_per_layer(tmp_path):
    ...
    pkg = tmp_path / "pkg"
    pkg.mkdir()
    c0 = write_bundle_package(pkg, n_layers=1, n_exp=4, out=6, cols=4, seed=11,
    ...
    c1 = make_layer_components(4, out=6, cols=4, seed=13)
    bundle, geo = assemble_layer_bundle(
        c1, {p: 4 for p in ("gate_proj", "up_proj", "down_proj")},
